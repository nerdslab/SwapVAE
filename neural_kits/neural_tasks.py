...
def compute_angle_accuracy_wlabels(net, classifier, data, transform=None, device='cpu'):
    r"""Evaluates the angle prediction performance of the network.
    Args:
        net (torch.nn.Module): Frozen encoder.
        classifier (torch.nn.Module): Linear layer.
    ...
    # feed to classifier
    with torch.no_grad():
        representation = net(x, one_hot).detach()
        pred_cos_sin = classifier(representation).detach().clone()
...
def train_angle_classifier_wlabels(net, classifier, data_train, data_val, optimizer, transform=None,
                           transform_val=None, batch_size=256, num_epochs=10, device='cpu'):
    r"""Trains linear layer to predict angle.
    Args:
        net (torch.nn.Module): Frozen encoder.
    ...
    for epoch in tqdm(range(num_epochs), disable=True):
        classifier.train()
        for x, _, label, cls in batch_iter(*data_train, batch_size=batch_size):
            x = x.to(device).squeeze()
            label = label.to(device).squeeze()
            ...
            one_hot = torch.zeros(x.shape[0], 8).to('cuda')  # hard code here, label class 8 in total
            one_hot[torch.arange(x.shape[0]), cls] = 1
            ...
            # forward
            with torch.no_grad():
                representation = net(x, one_hot).detach().clone()
                representation = representation.view(representation.shape[0], -1)
        ...
        # compute classification accuracies
        acc_train, delta_acc_train = compute_angle_accuracy_wlabels(net, classifier, data_train, transform=transform_val,
                                                            device=device)
        acc_test, delta_acc_test = compute_angle_accuracy_wlabels(net, classifier, data_val, transform=transform_val,
                                                          device=device)
